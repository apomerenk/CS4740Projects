...
def generateallwords():
	...
	for filename in os.listdir(os.getcwd()+'/data_corrected/classification task'):
		...
		if not filename =='test_for_classification':
			# print filename
			words=generatedirectorywords(os.getcwd()+'/data_corrected/classification task/'+filename+'/train_docs', allwords)
			allwords=words
	return words
# Generate the words in one directory
def generatedirectorywords(directory,currentwords):
